...
import { Users, DollarSign, Zap, TrendingUp, Clock, CheckCircle, XCircle, AlertCircle, Zap as ActivateIcon } from 'lucide-react'
...
async function activateUser(formData: FormData) {
  ...
  // 2. Log billing record
  await admin.from('billing_records').insert({
    user_id: userId,
    amount_usd: 21.62, // €19.90 ≈ $21.62
    payment_date: now.toISOString(),
    subscription_period_start: now.toISOString(),
    subscription_period_end: subscriptionEndsAt.toISOString(),
    status: 'paid',
    notes: 'Manual activation by admin',
  })
...
export default async function AdminBillingPage() {
  ...
  const statCards = [
    { label: 'Monthly revenue',    value: `€${monthlyRevenueEur.toFixed(2)}`,         sub: `${activeUsers.length} active subscriber${activeUsers.length !== 1 ? 's' : ''}`, icon: DollarSign, color: '#34D399' },
    { label: 'Token cost (all time)', value: `$${totalTokenCostUsd.toFixed(3)}`,       sub: `Avg $${avgCostPerUser.toFixed(3)}/user`,   icon: Zap,         color: '#F59E0B' },
    { label: 'Tokens this month',   value: totalTokensThisMonth.toLocaleString(),      sub: `${tokenRows.length} queries tracked`,      icon: TrendingUp,  color: '#60A5FA' },
    { label: 'User breakdown',      value: allUsers.length.toString(),                 sub: `${trialUsers.length} trial · ${activeUsers.length} active · ${expiredUsers.length} expired`, icon: Users, color: 'var(--accent)' },
  ]
    ...
    <div style={{ padding: '2.5rem', maxWidth: '1100px' }}>
      ...
      <div style={{ marginBottom: '2.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.875rem' }}>
          ...
          <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>Sorted by token usage ↓</span>
        </div>

        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          {/* Header */}
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 120px', padding: '0.6rem 1.25rem', borderBottom: '1px solid var(--border)', background: 'var(--surface-raised)' }}>
            {['Email', 'Status', 'Tokens (month)', 'Cost (USD)', 'Days left', 'Action'].map(h => (
          ...
          {allUsers.length === 0 ? (
            ...
            const endDate  = isActive ? u.subscription_ends_at : u.trial_ended_at
            ...
            return (
              <div
                key={u.id}
                style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 120px', padding: '0.75rem 1.25rem', borderBottom: isLast ? 'none' : '1px solid var(--border)', alignItems: 'center' }}
              >
                ...
                <span>
                  <StatusBadge status={u.subscription_status ?? 'expired'} />
                </span>
                ...
                <span>
                  {isActive ? (
                    ...
                    <form action={activateUser}>
                      ...
                      >
                        <ActivateIcon size={10} strokeWidth={2.5} /> Activate
                      </button>
                    </form>
                  )}
                </span>
      ...
      <div>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '0.6rem 1.25rem', borderBottom: '1px solid var(--border)', background: 'var(--surface-raised)' }}>
          ...
          {(billingRecords ?? []).length === 0 ? (
            <div style={{ padding: '1.5rem', color: 'var(--text-muted)', fontSize: '0.875rem' }}>
              No payments yet — this will populate when you activate a user.
            </div>
            ...
            return (
              <div
                key={r.id}
                style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '0.75rem 1.25rem', borderBottom: isLast ? 'none' : '1px solid var(--border)', alignItems: 'center' }}
              >
                <span style={{ fontSize: '0.85rem', color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingRight: '1rem' }}>
