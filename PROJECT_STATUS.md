...
│       │   │   ├── actions/patterns/      ← ✅ GET — fetches last 50 actions, calls Sonnet 4.6 to detect recurring workflow patterns, returns pattern or null; confidence threshold 0.75
...
│       ├── locales/
│       │   ├── en.json            ← ✅ English strings + palette.hint (8 context hints) + palette.pattern (workflow suggestion banner) (web)
│       │   └── ru.json            ← ✅ Russian strings + palette.hint (8 context hints) + palette.pattern (workflow suggestion banner) (web)
│       └── lib/
│           ├── supabase.ts        ← ✅ Done (server + user + admin clients)
│           ├── auth.ts            ← ✅ Done (requireAuth: Bearer token + cookie fallback, jsonError, jsonOk)
│           ├── assembler.ts       ← ✅ Done (instructions + skills + workflow memory + context tray injection + document-level action routing + scheduled skills awareness + pin a response + vision capability + save_file as 6th action)
...
└── app/                       ← Electron app (Windows desktop) — core working ✅
    └── src/
        ├── main/
        │   ├── index.ts           ← ✅ Main process, IPC handlers, stream proxy, resolve-file-refs IPC, silent token refresh on 401, scheduler init + refresh on login, result window cleanup on quit, vision IPC (capture-screenshot-now, get/set-auto-vision)
        ...
        │   ├── context-detector.ts ← ✅ Active app, file path, clipboard text, screenshot (opt-in only via captureVision param — default false; captureScreenshot() exported for on-demand IPC use)
        ...
        │   ├── store.ts           ← ✅ Persistent local store (authToken, refreshToken, prefs, contextTray, hotkey, dismissedPatterns with 7-day TTL, autoVision default false)
...
│   └── resultWindow.ts    ← ✅ Standalone always-on-top result window (560×480, bottom-right); opens when user clicks "View full" on notification; reuses window if already open
        ├── preload/
        │   ├── index.ts           ← ✅ IPC bridge (electronAPI on window) + setRefreshToken + getHotkey/setHotkey + isPatternDismissed + dismissPattern + openExternal + captureScreenshotNow + getAutoVision/setAutoVision + save_file Action type
│   └── result.ts          ← ✅ Minimal preload for result window (onPayload, close, openExternal)
        └── renderer/src/
            ...
            │   └── i18n.ts            ← ✅ i18next config (EN/RU, localStorage detection)
            ├── locales/
            │   ├── en.json            ← ✅ English strings + palette.hint (8 context hints) + palette.pattern (workflow suggestion banner) + palette.vision (read my screen / auto screen buttons) + palette.actions.save_file
            │   └── ru.json            ← ✅ Russian strings + palette.hint (8 context hints) + palette.pattern (workflow suggestion banner) + palette.vision (read my screen / auto screen buttons) + palette.actions.save_file
            ├── components/
            │   ├── CommandPalette.tsx ← ✅ Overlay UI + SSE streaming + actions + skills + conversation tracking + context tray + action logging + multiline input + amber vision indicator + i18n + file search indicator + truncation-safe action parsing + context hints (no-selection state) + file attachment (native dialog picker, fileRef injection, amber SVG paperclip, filename chip) + custom hotkey recorder (footer button, capture phase keydown, auto-confirm, amber recording state, green saved flash) + semantic vision gate (shouldUseVision: no hardcoded app names; sends clipboardText + screenshot separately) + skills strip auto-expand (palette--expanded on showSkillStrip, no maxHeight on strip) + AI workflow pattern detection (fires on palette open, 1-hour in-memory cache, amber suggestion banner, Yes/Dismiss with 7-day suppress) + manual/auto vision control ("read my screen" on-demand capture button, "auto screen: on/off" footer toggle persisted via store.ts, default off)
...
- [x] AI Learns Your Workflow — Sonnet 4.6 analyses last 50 actions on palette open (1-hour cache); detects recurring multi-step workflows; amber suggestion banner with "Create skill →" / "Dismiss"; skill editor pre-filled with AI-generated name + prompt; dismiss suppresses for 7 days (store.ts); no Supabase schema changes ← **DONE**
...
- [x]Allows Claude to create a file on disk. The agent emits an XML‑style action `<action type="save_file">filename.ext ← **DONE**
...
52. ✅ Semantic vision gate — `shouldCaptureScreenshot` + `VISUAL_APPS` hardcoded list removed; replaced with `shouldUseVision()` that ignores app name entirely; sends whatever screenshot was captured (manual or auto) whenever no fileRefs/trayClips present; decision is purely about context richness not app identity ← **DONE** (superseded by #59 — capture itself is no longer unconditional)
...
58. ✅ AI Learns Your Workflow — Sonnet 4.6 detects recurring multi-step workflows from last 50 completed actions; new `/api/actions/patterns` endpoint (GET); fires on every palette open with 1-hour in-memory cache (low cost: ~10–15 calls/user/month); amber suggestion banner shown in idle palette when confidence ≥ 0.75; "Create skill →" opens `/dashboard/skills/new` pre-filled with suggested name + prompt; "Dismiss" suppresses pattern for 7 days via store.ts (DismissedPattern + isPatternDismissed/dismissPattern helpers); IPC wired in main/index.ts + preload/index.ts; fully bilingual EN/RU; no Supabase schema changes needed ← **DONE**
59. ✅ Manual/Auto Vision Control — security fix from beta feedback: screen capture was unconditional on every palette open. `getContext(captureVision: boolean = false)` in `context-detector.ts` now only captures when explicitly told to; `captureScreenshot()` exported standalone for on-demand use. New `autoVision` flag in `store.ts` (default **false**). Renderer: "read my screen" button in `CommandPalette.tsx` triggers one-off capture via `captureScreenshotNow` IPC for the current query only; footer toggle "auto screen: on/off" persists the `autoVision` preference via `setAutoVision`/`getAutoVision` IPC. `main/index.ts` reads `store.getAutoVision()` at all 3 `getContext()` call sites (tray click, hotkey trigger, `get-context` IPC handler) and exposes 3 new handlers: `capture-screenshot-now`, `get-auto-vision`, `set-auto-vision`. `windows.ts` unchanged — `showPalette()` only receives the already-resolved context, it never captures itself. Existing beta users default to manual-only on update (no `autoVision` key in their old `store.json` → falls back to `false`) ← **DONE**
60. **File Save action architecture**: `save_file` is the 6th action type, added across the entire action pipeline. Claude emits `<action type="save_file">filename.ext|file content</action>` — `|` separates the suggested filename from the content (safe separator since filenames cannot contain `|` on Windows). `parseActionFromResponse` in `CommandPalette.tsx` splits on the first `|` to extract both. The action requires confirmation (shows "Save file: filename.ext" confirm button). On confirm, `actions.ts` calls `dialog.showSaveDialog` defaulting to the user's Downloads folder with the AI-suggested filename, then writes the file with `fs.promises.writeFile`, and opens the containing folder with `shell.openPath`. The assembler system prompt was updated from FIVE to SIX actions, the "cannot create files" limitation removed, and a new example added. ← **DONE**
...
- **AI Learns Your Workflow**: `/api/actions/patterns` (GET) fetches last 50 `completed` actions from Supabase, sends to Sonnet 4.6 with a structured prompt asking for repeating multi-step workflow patterns; returns `{ pattern: { name, description, suggested_prompt, apps_involved, confidence } | null }`; confidence threshold 0.75. Called on every palette open with a 1-hour in-memory cache (`patternCacheRef`) — typical user triggers ~10–15 real API calls/month (negligible cost). Amber banner renders in idle palette state when a pattern is detected; "Create skill →" opens `/dashboard/skills/new` pre-filled via URL params; "Dismiss" calls `dismissPattern(hash)` IPC → `store.ts` stores hash with 7-day expiry; pattern won't re-appear until TTL expires. Three new IPC channels: `open-external` (shell.openExternal), `is-pattern-dismissed`, `dismiss-pattern`. No Supabase schema changes — dismiss state is local only.
...
- **Manual/Auto Vision Control architecture**: prior to v1.0.10, `context-detector.ts` captured the screen on every single palette open regardless of need — flagged as a security concern during beta (capture happens before the user even sees the palette, so it could pick up sensitive content behind it). Fix: `getContext()` takes a `captureVision` param, default `false`; capture only fires when that's `true`. Two paths set it to `true`: (1) the `autoVision` preference in `store.ts` (off by default, toggled via the footer "auto screen: on/off" button), checked by `main/index.ts` at all 3 call sites before calling `getContext()`; (2) the renderer's "read my screen" button, which bypasses `getContext()` entirely and calls the exported `captureScreenshot()` directly via the `capture-screenshot-now` IPC handler, merging the result into the existing `context` state for that one query. `shouldUseVision()` in `CommandPalette.tsx` is unchanged — it already gated on `screenshotBase64` being present, so it works identically whether that came from auto-capture or a manual click.
- **File Save action architecture**: `save_file` is the 6th action type, added across the entire action pipeline. Claude emits `<action type="save_file">filename.ext|file content</action>` — `|` separates the suggested filename from the content (safe separator since filenames cannot contain `|` on Windows). `parseActionFromResponse` in `CommandPalette.tsx` splits on the first `|` to extract both. The action requires confirmation (shows "Save file: filename.ext" confirm button). On confirm, `actions.ts` calls `dialog.showSaveDialog` defaulting to the user's Downloads folder with the AI-suggested filename, then writes the file with `fs.promises.writeFile`, and opens the containing folder with `shell.openPath`. The assembler system prompt was updated from FIVE to SIX actions, the "cannot create files" limitation removed, and a new example added.

---

*Last updated: v1.0.11 — File Save action shipped (save_file as 6th action: native Save dialog, defaults to Downloads, opens folder on save). Palette UI fixes: app badge truncation, always-visible input separator, context strip padding.
Workflow memory ✅, Action history ✅, Screenshots ✅, Skill templates ✅, Token tracking ✅, Trial/subscription schema ✅, Admin billing dashboard ✅, Usage analytics ✅, Subscription activation ✅, Windows installer ✅, Auto-updater ✅, Semantic skill filtering ✅, Dashboard download modal ✅, Multilingual UI ✅, Context Tray ✅, Action logging ✅, Multiline input ✅, Vision indicator ✅, Schema sync ✅, Actions counter ✅, File search as context ✅, Document action routing ✅, Session persistence ✅, Context hints ✅, File attachment ✅, Login performance ✅, Pin a Response ✅, Smart model routing (Haiku/Sonnet) ✅, Palette smooth expand ✅, Semantic model router ✅, Custom hotkey ✅, Semantic vision gate ✅, Semantic clipboard relevance ✅, Auto-updater on palette open ✅, Skills strip auto-expand ✅, Scheduled Skills ✅, Calendar Integration (iCal) ✅, AI Learns Your Workflow ✅, Manual/Auto Vision Control ✅, File Save action ✅, Palette UI polish ✅
