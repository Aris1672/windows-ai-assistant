...
└── app/                       ← Electron app (Windows desktop) — core working ✅
    └── src/
        ...
        │   ├── context-detector.ts ← ✅ Active app, file path, clipboard text, screenshot (always captured unconditionally)
        ...
        │   ├── store.ts           ← ✅ Persistent local store (authToken, refreshToken, prefs, contextTray, hotkey, dismissedPatterns with 7-day TTL)
...
│   └── resultWindow.ts    ← ✅ Standalone always-on-top result window (560×480, bottom-right); opens when user clicks "View full" on notification; reuses window if already open
        ├── preload/
        │   ├── index.ts           ← ✅ IPC bridge (electronAPI on window) + setRefreshToken + getHotkey/setHotkey + isPatternDismissed + dismissPattern + openExternal
...
52. ✅ Semantic vision gate — `shouldCaptureScreenshot` + `VISUAL_APPS` hardcoded list removed; replaced with `shouldUseVision()` that ignores app name entirely; screenshot always captured, sent whenever no fileRefs/trayClips present; decision is purely about context richness not app identity ← **DONE**
